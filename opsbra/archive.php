...
?>
	...
	<?php else :
		...
		if ( is_category() ) { // If this is a category archive
			printf("<h2 class='center'>".__("Desculpe, mas nenhum conteúdo foi encontrado no canal %s", 'kubrick').'</h2>', single_cat_title('',false));
		} else if ( is_date() ) { // If this is a date archive
			echo('<h2>'.__("Desculpe, mas nenhum conteúdo foi encontrado", 'kubrick').'</h2>');
		} else if ( is_author() ) { // If this is a category archive
			$userdata = get_userdatabylogin(get_query_var('author_name'));
			printf("<h2 class='center'>".__("Desculpe, mas nenhum conteúdo foi encontrado no canal %s", 'kubrick')."</h2>", $userdata->display_name);
		} else {
			echo("<h2 class='center'>".__('Desculpe, mas nenhum conteúdo foi encontrado.', 'kubrick').'</h2>');
		}
	  get_search_form();
	endif;
?>
